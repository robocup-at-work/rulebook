...
\TODO{General Notes/Suggestions:}
\begin{itemize}

	\item apply common and consistent use of the keywords \textit{test}/\textit{task} and \textit{run}/\textit{period}.
	
	\item add lines between rows of Table~\ref{tab:Instances}	 and Table~\ref{tab:InstancePoints} to better distinguish between the particular rows. And maybe use double-lines for the existing single horizontal lines to better distinguish the respective groups. 
	
	\item rename "Assembly Award" (Chapter~\ref{cha:AssemblyAward}) to "Technical Challenges" and add "Assembly" as subsection.
	
	\item move subsections of Section~\ref{sec:Robots} (i.e. Section~\ref{ssec:RobotDesignAndConstraints} and \ref{ssec:RobotBehaviorAndSafety}) and the Section itself to Section~\ref{sec:AssumptionsAboutRobots}.
	...
	\item add description for the technical challenge on object recognition.
	
	\item rename CBT into something more general (which covers both, conveyor belt and rotating table).
	
	\item cross-check instances and points (see Tabel~\ref{tab:Instances} and \ref{tab:InstancePoints}).
	
	\item do teams get negative points each time they cross a barrier tape or only once per barrier tape?	
...
\chapter{Assembly Award} \label{cha:AssemblyAward}
\input{./general_rules/Assembly_Award.tex}
