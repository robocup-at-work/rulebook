...
The manipulation objects \revcha{in this test include the objects specified in Table \ref{tab:manipulation_objects}}{used in this test are specified by the instances defined in Table~\ref{tab:Instances}}.
...
\begin{itemize}
	...
	\item \revadd{A configuration in which the objects need to be placed at the destination service area}
	\item \revadd{A list of objects to manipulated from the source to the destination service area}
...
\end{center}
\par
Further configurations may be specified in future. Note, that the pose \revdel{and orientation} of each object in the destination service area is left unspecified and can be chosen by the robot, i.e. the objects do not need to be put in a line.
...
%
 ...
 \item \revdel{The team has at most 5 minutes to complete the task.} The time is stopped when the robot has completed the task by placing the last object of the task specification. If a team cannot complete the task within \revcha{5 minutes}{the designated time}, the run will be stopped \revdel{after 5 minutes}.
 \end{itemize}

\todo{check the specific time definitions} \note{The time will be definied in the instances. So, they do not need to be specified here, right? (Fred)}
%
%
% \subsection{Scoring}
