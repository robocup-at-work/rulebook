...
\item The objects are placed on the conveyor before the run starts\revdel{, for low complexity by the team or for medium and high by a referee.}.
...
\item \revdel{A team has a time period of 10 minutes to complete a run.} The time is stopped when the robot has completed the task.
