...
\lucas{Is it allowed to touch another object or container, decoy with the manipulator while the grasping Process.}
...
There is a Placement Deduction that will be subtracted from the points of the successfull Grasping. The Placement Deduction will we applied at the following situations, if multiple situations happen while the same placing process the Placement Deduction will applied only once.:
...
The placement process starts when the Manipulator with the Object enters the Manipulation Zone  and ends when the placed Object doesn't move anymore and the Manipulator has left the Manipulation Zone. In thiese cases the Manipulation Zone has an infinte height.
...
If a placed Object is either incorrect or the corresponding Service Area is incorrect the Placement is considered as an Incorrect Placement. If at this Incorrect Placement a Placement Deductions occurs then the amount of points for the Placement Deduction will be added to the penalty of the Incorrect Placement.  
\lucas{Penalty, Incorrect Placement?? Placement Deduction also aplly at incorrect Placement?}
...
At the Precision Placement Table it is allowed to poke with the Object in the Gripper to place the Object into the cavity. This  ensures the use of a force sensor for placing the Object into the cavity.
...
A successfull Placement of the Object into the cavity will rewarded with a successfull Precision Placement. If the Object is not falling down through the cavity but is lying at the end of the run at the corect cavity it will be rewarded with an successfull Cavity Placement. However The Object has to be lying with his complete form within the correct cavity. 

\lucas{Incorrect placement of the Object into/on the wrong cavity, 0points or penalty??}
