...
\textcolor{red}{The OC provides a power supply and public LAN cable in the competion arena in order to reduce
...
\begin{itemize}
	...
	\item \textcolor{red}{\sout{Restart used: \hfill -200 points}}
	\item \textcolor{red}{Delivering a wrong manipulation object to service area \hfill -50 points }
\end{itemize}

\textcolor{red}{Collisions of the manipulator with the upside of the service area 
are allowed.}

\textcolor{red}{old version \textit{
...
\par
\textcolor{red}{If a team has decided to run a task while considering virtual walls
and the robot's projection touches a barrier tape it counts as a collision.}

\textcolor{red}{old version \textit{If the projection of any part of a robot 
touches a barrier tape marking a virtual wall it counts as a collision.}}
...
\textcolor{red}{One hour before a test the OC requests the capability of
each team to participate. If a team chancels its participation at this
point of time, it cannot withdraw this decision during the test.}
The order in which the teams have to perform a run is determined by a draw by
the OC. The order will be made public at least \textcolor{red}{45 minutes} before slot of the
particular test. \textcolor{red}{Additionally, at this point in time each team 
has the opportunity to declare its general ignorance of barrier tapes. In this 
case, it accepts the general penalty listed in Tab.~\ref{tab:InstancePoints}.}
...
A run ends when
\begin{itemize}
	\item the duration for the given test has passed \textcolor{red}{(indicated by a sound signal of the refbox)},
...
\begin{itemize}
	...
	\item \textcolor{red}{Scores that are received after a restart are reduced by a factor of 0.75}
