...
TODO: Forbid flying robots (i dont want any drones here)
...
\begin{figure} [h!]
	\begin{center}
		...
		\subfloat{\includegraphics[width=0.3\textwidth]{./images/robots/tyrolics2.jpg}} \hfill
...
The height of a physical Wall must be not less than $20\si{\centi\meter}$ and not more than $40\si{\centi\meter}$.
...
\begin{figure} [h!]
	...
	\end{center}
	\caption{left: 0cm table with arbitrary surface (approx. 1\si{\centi\meter} thick); right: PPT-table which is a standard 10\si{\centi\meter}-table with cavities}
	\label{fig:ws}
\end{figure}
...
If the robot touches an Object while navigating, this will be handled as a Major Collision. Examples for $0\si{\centi\meter}$-Table are shown in figure \ref{fig:0cmws}.

If the service area located on a table is just used for pick and place purpose it will be referenced as \iaterm{Service areas placed on $0\si{\centi\meter}$, $5\si{\centi\meter}$, $10\si{\centi\meter}$ and $15\si{\centi\meter}$-tables used for pic and place operations}{workstation}. 
...
\martin{same speed for all teams or changed after each team??}
\lucas{same speed for all teams}
\begin{figure}[h!]
	\centering
...
\martin{Decoys placement, surface, obstacle placement are done by referees.  Decoys could also be standard objects from the rulebook.}
